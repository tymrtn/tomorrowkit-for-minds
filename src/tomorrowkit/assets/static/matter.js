...
(function () {
  ...
  function textOrPlaceholder(text) {
    ...
    return trimmed ? trimmed : "(nothing recorded yet)";
  }

  function describeMatterAndBrief() {
    var lines = [
      "--- My invention, as recorded so far ---",
      "Working title: " + textOrPlaceholder(doc.title),
      "Stage: " + (STAGE_LABELS[doc.stage] || doc.stage),
      "Problem and approach: " + textOrPlaceholder(doc.problem_summary),
      "Immediate goal: " + textOrPlaceholder(doc.goal)
    ];
    if (doc.known_dates.length) {
      lines.push("Important dates:");
      doc.known_dates.forEach(function (d) {
        lines.push("- " + textOrPlaceholder(d.label) + ": " + textOrPlaceholder(d.date_text) + (d.note ? " (" + d.note + ")" : ""));
      });
    }
    lines.push(
      "",
      "--- My invention brief ---",
      "Problem: " + textOrPlaceholder(doc.brief.problem),
      "",
      "Proposed mechanism: " + textOrPlaceholder(doc.brief.mechanism),
      "",
      "Intended result: " + textOrPlaceholder(doc.brief.intended_result),
      "",
      "Alternatives and variations: " + textOrPlaceholder(doc.brief.alternatives),
      "",
      "Open questions: " + textOrPlaceholder(doc.brief.open_questions)
    );
    return lines.join("\n");
  }

  function describeMapAsText() {
    var lines = ["--- My invention map ---"];
    if (!doc.map_nodes.length) {
      lines.push("(the map is empty)");
      return lines.join("\n");
    }
    var nodeLabelById = {};
    doc.map_nodes.forEach(function (node) {
      nodeLabelById[node.node_id] = node.label || "(unnamed)";
      var line = "- " + (node.label || "(unnamed)") + " [" + (NODE_KIND_LABELS[node.kind] || node.kind) + "]";
      if (node.note) { line += " -- " + node.note; }
      lines.push(line);
    });
    if (doc.map_edges.length) {
      lines.push("Connections:");
      doc.map_edges.forEach(function (edge) {
        var source = nodeLabelById[edge.source_node_id];
        var target = nodeLabelById[edge.target_node_id];
  ...
  function describeLibraryAsText() {
    var lines = ["--- My reference library ---"];
    if (!doc.references.length) {
      lines.push("(the library is empty)");
      return lines.join("\n");
    }
    doc.references.forEach(function (entry) {
      var line = "- " + entry.title + " [" + SOURCE_TYPE_LABELS[entry.source_type] + ", " +
        RELATIONSHIP_LABELS[entry.relationship] + ", " + VERIFICATION_LABELS[entry.verification_state] + "]";
      if (entry.citation) { line += " " + entry.citation; }
      if (entry.relevance_note) { line += " -- " + entry.relevance_note; }
      lines.push(line);
    });
    return lines.join("\n");
  }

  // What each checkpoint's copied prompt carries along, so the user's agent
  // always sees the current record without any manual pasting.
  var HARVEST_ATTACHMENTS = {
    intake: { parts: ["brief"], label: "your current brief" },
    prospecting: { parts: ["brief"], label: "your current brief" },
    drafting: { parts: ["brief", "map"], label: "your current brief and map" },
    adversarial: { parts: ["brief", "map", "library"], label: "your current brief, map, and reference library" }
  };

  function composeHarvestPrompt(checkpoint) {
    var attachment = HARVEST_ATTACHMENTS[checkpoint.checkpoint_id] || { parts: ["brief"] };
    var sections = [checkpoint.agent_prompt, describeMatterAndBrief()];
    if (attachment.parts.indexOf("map") !== -1) { sections.push(describeMapAsText()); }
    if (attachment.parts.indexOf("library") !== -1) { sections.push(describeLibraryAsText()); }
  ...
  function renderHarvest() {
    ...
    doc.harvest.forEach(function (checkpoint) {
      ...
      card.innerHTML =
        ...
        '<div class="prompt-well">' + escapeHtml(checkpoint.agent_prompt) + "</div>" +
        '<div style="display:flex; gap:10px; align-items:center; flex-wrap:wrap;">' +
          '<button class="btn tiny" data-role="copy">Copy prompt for your agent</button>' +
          '<span class="hint" data-role="copy-status" style="margin:0;">' +
            "Copying attaches " + escapeHtml((HARVEST_ATTACHMENTS[checkpoint.checkpoint_id] || {}).label || "your current brief") + "." +
          "</span>" +
        "</div>" +
        '<label class="field">What came out of it</label>' +
        '<textarea data-role="notes" rows="3" placeholder="Paste or summarize the results you approved…">' +
      ...
      card.querySelector('[data-role="copy"]').addEventListener("click", function () {
        var statusEl = card.querySelector('[data-role="copy-status"]');
        // Composed at click time so the attached record is always current.
        var composedPrompt = composeHarvestPrompt(checkpoint);
        copyTextToClipboard(composedPrompt, function (ok) {
          if (ok) {
            statusEl.textContent = "Copied with your current record — paste it into your agent's chat.";
          } else {
            var well = card.querySelector(".prompt-well");
            well.textContent = composedPrompt;
            var range = document.createRange();
            range.selectNodeContents(well);
            var selection = window.getSelection();
            selection.removeAllRanges();
            selection.addRange(range);
            statusEl.textContent = "Selected above — press Ctrl+C (or Cmd+C) to copy.";
          }
        });
      });
