...
#
# Usage: create_reviewer_settings.sh [AUTOFIX_ENABLE] [CI_ENABLE] [VERIFY_CONVERSATION_ENABLE] [AUTOFIX_NITS] [VERIFY_ARCHITECTURE_ENABLE] [STOP_HOOK_ENABLE]
#
...
#   AUTOFIX_NITS               - 1: fix all issues, 0: only CRITICAL/MAJOR/MINOR
...
PROMPT_MAJOR='Please autofix as normal, except: 1. Never ask questions. You are running unattended and the user is not there to answer your questions. Instead, think hard about whether to accept each given patch. If you decide *not* to accept it, then create a *new* branch with that fix commit. Call the branch (current_branch_name)___(fix_description) *and be sure to push it remotely* then by sure to check the normal branch back out when you'\''re done. Also be sure to tell the user that you did this.  2. You only *have* to fix CRITICAL, MAJOR, and MINOR issues. If there are issues that you do NOT fix, append the json object(s) for those error(s) that were *not* fixed into .reviewer/outputs/autofix/unfixed/<current-git-hash>.jsonl  If so, be sure to mention those issues in your final summary as well.'

if [[ "$AUTOFIX_NITS" == "1" ]]; then
    PROMPT="$PROMPT_ALL"
else
    PROMPT="$PROMPT_MAJOR"
fi
